...
TEST(TextFormattersTest, LiteralInfo) {
  EXPECT_EQ(R"({sign None, kind NanPayload, base Hex, has_underscores No})",
            format(LiteralInfo{Sign::None, LiteralKind::NanPayload, Base::Hex,
                               HasUnderscores::No}));
}
...
TEST(TextFormattersTest, VarList) {
  EXPECT_EQ(R"([0 1 2 $a])", format(VarList{Var{Index{0}}, Var{Index{1}},
...
TEST(TextFormattersTest, FunctionType) {
  EXPECT_EQ(R"({params [i32], results [f32]})", format(FunctionType{
...
TEST(TextFormattersTest, MemArgImmediate) {
  ...
  EXPECT_EQ(R"({align 4, offset 0})", format(MemArgImmediate{u32{4}, u32{0}}));
}
...
TEST(TextFormattersTest, Instruction) {
  ...
  EXPECT_EQ(
      ...
      format(Instruction{Opcode::BrTable, BrTableImmediate{{}, Var{"$b"_sv}}}));
...
TEST(TextFormattersTest, GlobalDesc) {
  EXPECT_EQ(R"({name none, type const i32})",
            format(GlobalDesc{nullopt, GlobalType{VT_I32, Mutability::Const}}));
}
...
TEST(TextFormattersTest, ElementList) {
  ...
  EXPECT_EQ(
      ...
      format(ElementList{ElementListWithVars{ExternalKind::Function, {}}}));
}

TEST(TextFormattersTest, Table) {
  EXPECT_EQ(
      R"({desc {name none, type {min 1} funcref}, import none, exports [], elements none})",
      format(Table{TableDesc{nullopt, TableType{Limits{1}, RT_Funcref}}, {}}));
}
...
TEST(TextFormattersTest, Global) {
  EXPECT_EQ(
      ...
      format(Global{GlobalDesc{nullopt, GlobalType{VT_I32, Mutability::Const}},
                    ConstantExpression{},
                    {}}));
}
...
TEST(TextFormattersTest, Export) {
  EXPECT_EQ(
      ...
      format(Export{ExternalKind::Function, Text{"$a"_sv, 1}, Var{Index{0}}}));
}
...
TEST(TextFormattersTest, ModuleItem) {
  ...
  EXPECT_EQ(R"(export {kind func, name {text $a, byte_size 1}, var 0})",
            format(ModuleItem{Export{ExternalKind::Function, Text{"$a"_sv, 1},
                                     Var{Index{0}}}}));
  ...
  EXPECT_EQ(
      ...
      format(ModuleItem{ElementSegment{nullopt, SegmentType::Passive, {}}}));
...
TEST(TextFormattersTest, ScriptModule) {
  ...
  EXPECT_EQ(
      ...
      format(ScriptModule{nullopt, ScriptModuleKind::Binary, TextList{}}));
...
TEST(TextFormattersTest, Const) {
  ...
  // f32
  EXPECT_EQ(R"(f32 0)", format(Const{f32{}}));

  // f64
  EXPECT_EQ(R"(f64 0)", format(Const{f64{}}));
  ...
  // RefExternConst
  EXPECT_EQ(R"(ref.extern {var 0})", format(Const{RefExternConst{u32{}}}));
}

TEST(TextFormattersTest, ConstList) {
  EXPECT_EQ(R"([u32 0 u64 0 f32 0 f64 0])", format(ConstList{
                                                Const{u32{}},
                                                Const{u64{}},
                                                Const{f32{}},
                                                Const{f64{}},
...
TEST(TextFormattersTest, F32Result) {
  EXPECT_EQ(R"(f32 0)", format(F32Result{f32{}}));
...
TEST(TextFormattersTest, F64Result) {
  EXPECT_EQ(R"(f64 0)", format(F64Result{f64{}}));
...
TEST(TextFormattersTest, F32x4Result) {
  EXPECT_EQ(R"([f32 0 f32 0 f32 0 f32 0])", format(F32x4Result{}));
...
TEST(TextFormattersTest, F64x2Result) {
  EXPECT_EQ(R"([f64 0 f64 0])", format(F64x2Result{}));
...
TEST(TextFormattersTest, ReturnResult) {
  ...
  // F32Result
  EXPECT_EQ(R"(f32 f32 0)", format(ReturnResult{F32Result{}}));

  // F64Result
  EXPECT_EQ(R"(f64 f64 0)", format(ReturnResult{F64Result{}}));
  ...
  // F64x2Result
  EXPECT_EQ(R"(f64x2 [f64 0 f64 0])", format(ReturnResult{F64x2Result{}}));
...
TEST(TextFormattersTest, Assertion) {
  ...
  EXPECT_EQ(
      ...
      format(Assertion{AssertionKind::ActionTrap,
...
TEST(TextFormattersTest, Command) {
  ...
  EXPECT_EQ(
      ...
      format(Command{ScriptModule{nullopt, ScriptModuleKind::Text, Module{}}}));
...
TEST(TextFormattersTest, Script) {
  ...
  EXPECT_EQ(
      ...
      format(Script{Command{Register{Text{"\"hi\"", 2}, nullopt}},
