...
TEST_F(ValidMatchTest, IsSame_ReferenceType_Simple) {
  std::vector<ReferenceType> types{
      RT_Funcref,   RT_Externref,     RT_Anyref, RT_Eqref, RT_I31ref,
  ...
  IsSameTable(
      ...
      F  E              u     u     u     n     u  
      u  x  A     I     l     l     l     u     l     n
      n  t  n  E  3     l     l     l     l     l     u
      c  .  y  q  1  f  f                 l           l
      r  r  r  r  r  u  u  e  e  a  a        i  i     l
      e  e  e  e  e  n  n  x  x  n  n  e  e  3  3  
      f  f  f  f  f  c  c  t  t  y  y  q  q  1  1  0  0   */
      S, _, _, _, _, _, S, _, _, _, _, _, _, _, _, _, _,  // Funcref
      _, S, _, _, _, _, _, _, S, _, _, _, _, _, _, _, _,  // Externref
      _, _, S, _, _, _, _, _, _, _, S, _, _, _, _, _, _,  // Anyref
      _, _, _, S, _, _, _, _, _, _, _, _, S, _, _, _, _,  // Eqref
      _, _, _, _, S, _, _, _, _, _, _, _, _, S, _, _, _,  // I31ref
      _, _, _, _, _, S, _, _, _, _, _, _, _, _, _, _, _,  // ref func
      S, _, _, _, _, _, S, _, _, _, _, _, _, _, _, _, _,  // ref null func
      _, _, _, _, _, _, _, S, _, _, _, _, _, _, _, _, _,  // ref extern
      _, S, _, _, _, _, _, _, S, _, _, _, _, _, _, _, _,  // ref null extern
      _, _, _, _, _, _, _, _, _, S, _, _, _, _, _, _, _,  // ref any
      _, _, S, _, _, _, _, _, _, _, S, _, _, _, _, _, _,  // ref null any
      _, _, _, _, _, _, _, _, _, _, _, S, _, _, _, _, _,  // ref eq
      _, _, _, S, _, _, _, _, _, _, _, _, S, _, _, _, _,  // ref null eq
      _, _, _, _, S, _, _, _, _, _, _, _, _, S, _, _, _,  // ref i31
      _, _, _, _, _, _, _, _, _, _, _, _, _, _, S, _, _,  // ref null i31
      _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, S, _,  // ref 0
      _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, S,  // ref null 0
  });
}

TEST_F(ValidMatchTest, IsSame_Rtt) {
  std::vector<ValueType> rtts{
      VT_RTT_0_Func, VT_RTT_0_Extern, VT_RTT_0_Any,
  ...
  IsSameTable(
      types,
      {
          /*                               n     n     n           n  
                         ...
                         n  t  n  E  3     l     l     l     l     l     u
                      v  c  .  y  q  1  f  f                 l           l
          i  i  f  f  1  r  r  r  r  r  u  u  e  e  a  a        i  i     l
          3  6  3  6  2  e  e  e  e  e  n  n  x  x  n  n  e  e  3  3  
          2  4  2  4  8  f  f  f  f  f  c  c  t  t  y  y  q  q  1  1  0  0   */
          S, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, // I32
          _, S, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, // I64
          _, _, S, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, // F32
          _, _, _, S, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, // F64
          _, _, _, _, S, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, // V128
          _, _, _, _, _, S, _, _, _, _, _, S, _, _, _, _, _, _, _, _, _, _, // Funcref
          _, _, _, _, _, _, S, _, _, _, _, _, _, S, _, _, _, _, _, _, _, _, // Externref
          _, _, _, _, _, _, _, S, _, _, _, _, _, _, _, S, _, _, _, _, _, _, // Anyref
          _, _, _, _, _, _, _, _, S, _, _, _, _, _, _, _, _, S, _, _, _, _, // Eqref
          _, _, _, _, _, _, _, _, _, S, _, _, _, _, _, _, _, _, S, _, _, _, // I31ref
          _, _, _, _, _, _, _, _, _, _, S, _, _, _, _, _, _, _, _, _, _, _, // ref func
          _, _, _, _, _, S, _, _, _, _, _, S, _, _, _, _, _, _, _, _, _, _, // ref null func
          _, _, _, _, _, _, _, _, _, _, _, _, S, _, _, _, _, _, _, _, _, _, // ref extern
          _, _, _, _, _, _, S, _, _, _, _, _, _, S, _, _, _, _, _, _, _, _, // ref null extern
          _, _, _, _, _, _, _, _, _, _, _, _, _, _, S, _, _, _, _, _, _, _, // ref any
          _, _, _, _, _, _, _, S, _, _, _, _, _, _, _, S, _, _, _, _, _, _, // ref null any
          _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, S, _, _, _, _, _, // ref eq
          _, _, _, _, _, _, _, _, S, _, _, _, _, _, _, _, _, S, _, _, _, _, // ref null eq
          _, _, _, _, _, _, _, _, _, S, _, _, _, _, _, _, _, _, S, _, _, _, // ref i31
          _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, S, _, _, // ref null i31
          _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, S, _, // ref 0
          _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, S, // ref null 0
      });
}
...
TEST_F(ValidMatchTest, IsMatch_HeapType_Simple) {
  ...
  IsMatchTable(types,
               {
                   /*
                   Func  Ext.  Any   Eq    I31   0     1     */
                   SAME, ____, MTCH, ____, ____, ____, ____,  // Func
                   ____, SAME, MTCH, ____, ____, ____, ____,  // Extern
                   ____, ____, SAME, ____, ____, ____, ____,  // Any
                   ____, ____, MTCH, SAME, ____, ____, ____,  // Eq
                   ____, ____, MTCH, MTCH, SAME, ____, ____,  // I31
                   MTCH, ____, MTCH, ____, ____, SAME, ____,  // 0
                   ____, ____, MTCH, MTCH, ____, ____, SAME,  // 1
               });
}
...
TEST_F(ValidMatchTest, IsMatch_RefType_Simple) {
  ...
  IsMatchTable(
      types,
      {
          ...
          u  u  e  e  a  a        i  i     l     l
          n  n  x  x  n  n  e  e  3  3  
          c  c  t  t  y  y  q  q  1  1  0  0  1  1  */
          S, M, _, _, M, M, _, _, _, _, _, _, _, _,  // ref func
          _, S, _, _, _, M, _, _, _, _, _, _, _, _,  // ref null func
          _, _, S, M, M, M, _, _, _, _, _, _, _, _,  // ref extern
          _, _, _, S, _, M, _, _, _, _, _, _, _, _,  // ref null extern
          _, _, _, _, S, M, _, _, _, _, _, _, _, _,  // ref any
          _, _, _, _, _, S, _, _, _, _, _, _, _, _,  // ref null any
          _, _, _, _, M, M, S, M, _, _, _, _, _, _,  // ref eq
          _, _, _, _, _, M, _, S, _, _, _, _, _, _,  // ref null eq
          _, _, _, _, M, M, M, M, S, M, _, _, _, _,  // ref i31
          _, _, _, _, _, M, _, M, _, S, _, _, _, _,  // ref null i31
          M, M, _, _, M, M, _, _, _, _, S, M, _, _,  // ref 0
          _, M, _, _, _, M, _, _, _, _, _, S, _, _,  // ref null 0
          _, _, _, _, M, M, M, M, _, _, _, _, S, M,  // ref 1
          _, _, _, _, _, M, _, M, _, _, _, _, _, S,  // ref null 1
      });
}
...
TEST_F(ValidMatchTest, IsMatch_ReferenceType_Simple) {
  ...
  IsMatchTable(
      types,
      {
          /*                n     n     n           n  
          F  E              u     u     u     n     u  
          u  x  A     I     l     l     l     u     l     n     n
          n  t  n  E  3     l     l     l     l     l     u     u
          c  .  y  q  1  f  f                 l           l     l
          r  r  r  r  r  u  u  e  e  a  a        i  i     l     l
          e  e  e  e  e  n  n  x  x  n  n  e  e  3  3  
          f  f  f  f  f  c  c  t  t  y  y  q  q  1  1  0  0  1  1  */
          S, _, M, _, _, _, S, _, _, _, M, _, _, _, _, _, _, _, _, // Funcref
          _, S, M, _, _, _, _, _, S, _, M, _, _, _, _, _, _, _, _, // Externref
          _, _, S, _, _, _, _, _, _, _, S, _, _, _, _, _, _, _, _, // Anyref
          _, _, M, S, _, _, _, _, _, _, M, _, S, _, _, _, _, _, _, // Eqref
          _, _, M, M, S, _, _, _, _, M, M, M, M, S, M, _, _, _, _, // I31ref
          M, _, M, _, _, S, M, _, _, M, M, _, _, _, _, _, _, _, _, // ref func
          S, _, M, _, _, _, S, _, _, _, M, _, _, _, _, _, _, _, _, // ref null func
          _, M, M, _, _, _, _, S, M, M, M, _, _, _, _, _, _, _, _, // ref extern
          _, S, M, _, _, _, _, _, S, _, M, _, _, _, _, _, _, _, _, // ref null extern
          _, _, M, _, _, _, _, _, _, S, M, _, _, _, _, _, _, _, _, // ref any
          _, _, S, _, _, _, _, _, _, _, S, _, _, _, _, _, _, _, _, // ref null any
          _, _, M, M, _, _, _, _, _, M, M, S, M, _, _, _, _, _, _, // ref eq
          _, _, M, S, _, _, _, _, _, _, M, _, S, _, _, _, _, _, _, // ref null eq
          _, _, M, M, S, _, _, _, _, M, M, M, M, S, M, _, _, _, _, // ref i31
          _, _, M, M, _, _, _, _, _, _, M, _, M, _, S, _, _, _, _, // ref null i31
          M, _, M, _, _, M, M, _, _, M, M, _, _, _, _, S, M, _, _, // ref 0
          M, _, M, _, _, _, M, _, _, _, M, _, _, _, _, _, S, _, _, // ref null 0
          _, _, M, M, _, _, _, _, _, M, M, M, M, _, _, _, _, S, M, // ref 1
          _, _, M, M, _, _, _, _, _, _, M, _, M, _, _, _, _, _, S, // ref null 1
      });
}
...
TEST_F(ValidMatchTest, IsMatch_ValueType_Simple) {
  ...
  std::vector<ValueType> reference_types{
      VT_Funcref,   VT_Externref,     VT_Anyref, VT_Eqref, VT_I31ref,
